...
class TestTranslate:
    async def test_streams_chunks_as_sse(self, client, auth_headers):
    ...
    async def test_falls_back_to_server_key_when_no_user_key(self, client, auth_headers):
        ctx, fake = _patch_provider()
        with ctx:
            response = await client.post("/api/v1/translate", json={"text": "hello"}, headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_returns_403_when_no_key_and_fallback_disallowed(self, client, db, user, auth_headers):
